...
class _OTPViewState extends State<OTPView> {
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: OtpPinField(
                  textInputAction: TextInputAction.done,
                  showCustomKeyboard: false, // Use default keyboard
                  onSubmit: (newCode) {
                    setState(() {
                      code = newCode;
                    });
  ...
  void btnSubmit() {
    if (code.length != 6) {
      mdShowAlert(Globs.appName, "Please enter a valid 6-digit OTP.", () {});
      return;
    }
    ...
    print("Submitting OTP: $code"); // Debugging purpose
    serviceCallForgotVerify({"email": widget.email, "reset_code": code});
  }
    ...
    ServiceCall.post(
      ...
      withSuccess: (responseObj) async {
        ...
        if (responseObj[KKey.status] == "1") {
          ...
          mdShowAlert(
              Globs.appName,
              responseObj[KKey.message] as String? ?? "Verification failed",
              () {});
        }
      },
      failure: (err) async {
  ...
  void serviceCallForgotRequest(Map<String, dynamic> parameter) {
    setState(() {
      isLoading = true; // Show loading indicator for resend
    });
    ...
    ServiceCall.post(
      ...
      withSuccess: (responseObj) async {
        ...
        if (responseObj[KKey.status] == "1") {
          ...
          mdShowAlert(Globs.appName,
              responseObj[KKey.message] as String? ?? "Request failed", () {});
        }
      },
      failure: (err) async {
        setState(() {
